...
public class Server {
	...
	public Result createNewResult(@RequestBody String author) {
		...
		if (res == null) {
			res = new Result(author, 0L, 0L, env.getProperty("secretMessage", "no secret message yet..."));
		} else {
			res = new Result(author, res.getResult() + 1L, res.getResult2() + 2L, res.getHiddenSecret());
		}
